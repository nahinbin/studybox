...
def pomodoro_home(user_id):
    ...
    for sub in enrolled_subjects:
        ...
        if record:
            ...
            time_data[sub] = 0  # Default to 0 if no study time recorded
...
def saveTime(user_id, subject):
    """Save study time to database and accumulate with existing records"""
    ...
    if record:
        record.minutes += minutes  # Accumulate study time
    else:
